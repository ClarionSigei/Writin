...
def essay():
    ...
    if request.method == 'POST':
        ...
        # Use send_simulated_email instead of send_email
...
@app.route('/admin/send_message/<order_type>/<int:order_id>', methods=['POST'])
@admin_required
def send_admin_message(order_type, order_id):
    ...
    if order_type == 'general':
        ...
        if order and order.user_id:
            create_notification(order.user_id, 'new_message', 'New Message',
                                f'You have a new message regarding request {order.order_id_string}.',
                                url_for('view_order', order_type='general', order_id=order.order_id_string))
    elif order_type == 'book':
        order = BookClassInquiry.query.get(order_id)
        if order and order.user_id:
            create_notification(order.user_id, 'new_message', 'New Message',
                                f'You have a new message regarding inquiry {order.order_id_string}.',
                                url_for('view_order', order_type='book', order_id=order.order_id_string))
